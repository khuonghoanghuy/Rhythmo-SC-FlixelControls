...
class RuleScriptInterp extends hscript.Interp
{
	...
	override function makeKeyValueIterator(v:Dynamic)
	{
		...
		// if it already has correct structure, return as-is
		if (Reflect.hasField(v, "hasNext") && Reflect.hasField(v, "next"))
		{
			var hasNext = Reflect.field(v, "hasNext");
			var next = Reflect.field(v, "next");

			// Wrap them in vars to match Haxe's expected iterator shape
			return {
				hasNext: hasNext,
				next: next
			};
