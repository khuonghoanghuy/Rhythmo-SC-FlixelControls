...
class Achievements
{
	...
	public static function unlock(ach:String, stats:AchievementStats, ?onFinish:Void->Void):Void
	{
		if (!isUnlocked(ach))
		{
	...
	public static function createStat(?date:Date, ?song:String):AchievementStats
	{
	...
	public static function forget(ach:String):Void
	{
		if (isUnlocked(ach))
		{
	...
	public static function showAchievement(ach:String, ?onFinish:Void->Void):Void
	{
		...
		var achIcon:FlxSprite = new GameSprite(achBG.x + 10, achBG.y + 10).loadGraphic(Paths.image('achievements/$ach'));
